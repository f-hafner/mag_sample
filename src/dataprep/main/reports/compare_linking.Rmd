...
```{r}
res_inspect <- lapply(fields_to_process, function(x) compare(x,  "graduates", linker1, linker2, "19852015", inspect = TRUE))
names(res_inspect) <- fields_to_process
```

```{r}
d_biology <- res_inspect$biology
d_chemistry <- res_inspect$chemistry

d_biology |> filter(pqid %in% d_chemistry$pqid) |> head()

d_biology |> filter(pqid == 304765279)
d_chemistry |> filter(pqid == 304765279)

graduate_links |> filter(goid == 304765279)
```
...
dk <- map(
  .x = names(res_inspect),
  .f = ~res_inspect[[.x]] |> mutate(field = .x)
) |> 
  bind_rows()
```
...
across_fields_graduates <- across_fields_graduates |>
  ...
  summarise(
    ...
    across(all_of(c("nlink1", "nlink2", "n_links_final")),
           ~sum(.x))
  ) |>
  mutate(field = "Total")
...
df_table_graduates <-  res_combined_graduates %>%  
  select(field, same, only1, only2, diff, nlink1, nlink2, n_links_final) %>%
...
table_caption <- "Comparing the links for graduates across labellers"
table_label <- "compare_linking_graduates"
table_columns <- c("Field", "Same ID", "Only by 1", "Only by 2", "Different ID", "Labeller 1", "Labeller 2", "Combined")
footnote <- c(
  "The table compares the identified links from ProQuest to MAG across training sets by two different labellers.",
  "The unit of observation is the graduate in ProQuest.",
  "First, the columns headed by ``Fraction of links found'' pool the links from the two models together and report the fractions across",
  "four cases: ",
  "``Same ID'' are cases where the models trained on the different training sets find the same MAG identifier.",
  "``Only by 1'' and ``Only by 2'' are cases where only the model trained on either of the training sets found a link to MAG at all.",
  "``Different ID'' are cases where both models find links to MAG, but to different identifiers.",
  "Second, the columns headed by ``Number of links found'' report the total number of links found.",
  "``Labeller 1'' and ``Labeller 2'' are the number of links found by the two models trained on different training data.",
  "``Combined'' are the number of links after combining the links from the two model as described in the text.",
  "The last row reports the total across fields. The fractions are weighted by the number of graduates in the respective fields in ProQuest 1990--2015."
)
...
table_graduates <- df_table_graduates |>   
  ...
  kableExtra::add_header_above(
    header = c(" " = 1, "Fraction of links found" = 4, "Number of links found" = 3)
  ) |>
...
# TODO: report average across fields, weighted by number of proquest graduates (?) in each field?
```
