...
compare <- function(field, linktype, linker1, linker2, years){
 ...
 # load NormalizedName of AuthorId_1 and AuthorId_2
  ...
  links <- links %>% 
    mutate(namedist = stringdist(NormalizedName_1, NormalizedName_2, method="jw"),
           namedist_pq_1 = stringdist(NormalizedName_1, normalizedname_pq, method = "jw"),
           namedist_pq_2 = stringdist(NormalizedName_2, normalizedname_pq, method = "jw"))
  ...
  shares <- links %>% summarize(same = sum(samelink, na.rm=TRUE)/n_distinct(pqid),
                                ...
                                namedist_pq_1 = mean(namedist_pq_1 * difflink, na.rm=TRUE)/mean(difflink),
                                namedist_pq_2 = mean(namedist_pq_2 * difflink, na.rm=TRUE)/mean(difflink)
                                )
