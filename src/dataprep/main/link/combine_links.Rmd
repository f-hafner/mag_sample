...
combine <- function(field, linktype, linker1, linker2, years, inspect=FALSE){
 ...
 # load NormalizedName of AuthorId_1 and AuthorId_2
  ...
  if (linktype=="advisors"){
    colnames(links)[colnames(links) == "pqid"] <- "relationship_id"  # Rename the column which was names "c"
  }
  links = links[,c("relationship_id", "AuthorId", "link_score")]
  links = links[complete.cases(links)]
  fwrite(links, file=paste0(datapath, "linked_ids_temp/combined_",linktype, field, ".csv"))
  if (inspect){
    return(links)
  } 
}
