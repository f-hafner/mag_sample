...
GROUP BY AuthorId
"
# NOTE: why this works is a bit of a mystery:
  # I thought one would have to add "having rowid = min(rowid)" as described here
    # https://stackoverflow.com/questions/35489971/select-first-row-in-each-group-using-group-by-in-sqlite/35490165
  # but when doing this, I get out 0 records...
...
* some way to benchmark? e.g. total number of researchers per field and gender in a country (US?)
  * census? -- does not report doctoral degree
  * NCSES?
    * National Survey of College Graduates?
    * SESTAT -- covers since 1993; probably needs cleaning 
* rise of Asia -> more and more authors w/o gender assigned. What to do about it? 
* what is the rising fraction of Asians without a clear gender assigned? 
...
* Substantial fraction of authors with very low HHI
  * Is this a MAG measurement problem or is this genuine?
  * As of now, I could not find an obvious explanation for this
  * Implications of this fact
    * The field1 FEs may be "mismeasured" and thus attenuate the estimated FEs 
      towards 0, but this should not matter per se
    * It could bias the estimated FEs if the authors are wrongly assigned to fields
    * As a robustness check we can always drop those authors with very low HHIs
* Open questions
  * Is the exponential growth in number of authors real or a measurement problem of MAG?
    * Has it been documented in another data set?
    * Does it also hold for journal articles?
    * *Note*: the data are worldwide -- 
      difficult to benchmark against other non-bibliometric data sets
